...
1. Toy examples to present how to use the `farr` package.

2. An example close to an actual climate application to give an idea of how this method can be applied in climate sciences. We will show how to repoduce some results of @naveau_revising_2018.
...
In climate sciences, the $FAR$ (Fraction of Attributable Risk) has been introduced by @stott_human_2004 to provide a synthetic number for this comparison:
...
@hannart_causal_2016 also note that $\max(FAR, 0)$ can be also interpreted as the probability of necessary causation in Pearl's probabilistic causal theory [@pearl_probabilities_1999] 


### The $far$ (fraction of attributable risk for records)

@naveau_revising_2018 present statistical methods to estimate the FAR for a different class of events: records.

#### Records

@naveau_revising_2018 follow the definition of records provided in @resnick_extreme_1987.
...
Then, to obtain the $FAR$ for records, we just need to estime to estimate $p_{1,r}$: 
...
@naveau_revising_2018 provide a statiscal procedure to estimate $p_1,r$ which then leads to an estimation of the $far$ for records and its confidence interval.
The different elements of this procedure are described in the table below. We only want to provide a general idea of how the method is built. For the complete explanation and all the proofs, please refer to @naveau_revising_2018 .
...
@naveau_revising_2018 show that this estimator is asymptotically normal. Then,  $\hat{p}_{1,2}$ can be used to estimate $\theta$. The asympotic gaussianity
...
where $\hat{G}_m$ is a kernel empirical estimate of the CDF of $X$.
Then the Cox and Oakes [@cox_d_analysis_1984] test is used to assess whether $\hat{W_i}$ are distributed according to an exponential distribution. 
...
The function returns an object of class `("farrfit_exp", "farrfit")`.
...
`estim_farr.np`. It takes as arguments:
...
and it returns an object of class `("farrfit_np", "farrfit")`. 
It is a list containing the following elements: 
...
The non-parametric estimate of the $far$ does not performs as good as the parametric estimation. The theoretical $far$, for a return period $r = 20$, does not fall within the 90%-confidence interval. 
...
In the following, we will estimate the value of the $far$ for a return period $r_\theta = 1 + 1 / \sqrt\theta$. In @naveau_revising_2018, this specific return period was chosen because in theory, under the null hypothesis that $W$ follows an exponential distribution, this is the return period that maximize the probability of necessary and sufficient causation [@hannart_causal_2016].
